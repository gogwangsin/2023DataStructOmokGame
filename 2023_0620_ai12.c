	...
	for (int i = 0; i < 19; i++)
	{
		...
		for (k = 1; k < 19; k++)
		{
			...
			else if (list[i][k].combo == 3) // 3일때  3일때 한번 저장한거임 내가 볼땐 oBBOBO인데 OBBOBB
			{
				...
				if (list[i][startPoint].state == 'B') //whelp 처음이 검은색일때
				{
					if (startPoint - 1 >= 0 && (Game[i][startPoint - 1] == '+') && (list[i][startPoint - 1].max != 4)) // 공백일때
					{
						Whelp[i][startPoint - 1] = 'o'; // 막아!
					}
					if (endPoint + 1 <= 18 && (Game[i][endPoint + 1] == '+')) // 공백일때
				...
				{
					if (startPoint - 1 >= 0 && (Game[i][startPoint - 1] == '+') && (list[i][startPoint - 1].combo != 4)) // 공백일때
					{
						Bhelp[i][startPoint - 1] = 'o'; // 막아! -> B도와주는 맵 
					}
					if (endPoint + 1 <= 18 && (Game[i][endPoint + 1] == '+')) // 공백일때
	...
	for (int i = 18, k = 0; i > 0; i--, k++)
	{
		...
		for (int y = 1; y < 2 * k; y++)
		{
			if (y < 19 && i + y < 19) // Game[0+y][i+y]  i 18~0, k0 ++, y=0~2*k y가 증가하는 줄 
			{
				if (y - 1 == 0) // 맨 처음일때 
				{
					if (Game[i + y - 1][y - 1] == 'B') // y는 1이고 y-1이 0인 상황이 맨 처음 
					{
						countB = 1;
						sumB = countB;
						startPoint = y - 1;
						endPoint = y - 1;
						list[i + y - 1][y - 1].state = 'B';
						list[i + y - 1][y - 1].Y = y - 1 + i;
						list[i + y - 1][y - 1].X = y - 1;
						list[i + y - 1][y - 1].combo = 1;
					}
					else if (Game[i + y - 1][y - 1] == 'W')
					{
						countW = 1;
						sumW = countW;
						startPoint = y - 1;
						endPoint = y - 1;
						list[i + y - 1][y - 1].state = 'W';
						list[i + y - 1][y - 1].Y = i + y - 1;
						list[i + y - 1][y - 1].X = y - 1;
						list[i + y - 1][y - 1].combo = 1;
					}
					else if (Game[i + y - 1][y - 1] == '+')
					{
						nonePoint = y - 1;
						list[i + y - 1][y - 1].state = '+';
						list[i + y - 1][y - 1].Y = i + y - 1;
						list[i + y - 1][y - 1].X = y - 1;
					}
				}
				...
				if (Game[i + y][0 + y] == Game[i + y - 1][y - 1]) // 이전과 같을 때 BB ++ WW  기본형 Game[i][y].Game[0+y][i+y]
				{
					...
					else if (Game[y + i][y + 0] == '+') // 공백 2이상연속 nonepoint부터 돌멩이 나올때까지 k 조정 
					{
						if (y - 2 >= 0 && (Game[y - 2 + i][y - 2] == 'B'))
						{
							list[startPoint + i][startPoint].max = list[y - 2 + i][y - 2].combo; // 연속 공백 나오면 끊어야하는데 전에 있던거 max에 백업해야지 
						}
						else if (y - 2 >= 0 && (Game[y - 2 + i][y - 2] == 'W'))
						{
				...
				else if (Game[y + i][y] != Game[y - 1 + i][y - 1]) // WB +B BW +W B+ W+
				{
					if (Game[y + i][y] == 'B') // 전에 꺼 비교해서 설정해주기 
					{
						...
						else  if (y - 1 >= 0 && (Game[y - 1 + i][y - 1] == '+')) // AOB BOB 
						{
							if (countNone == 1) // 공백 하나일때 연속중 
							{
								countB++;
					...
					else if (Game[y + i][y] == '+') // coutnone 2면 끊어줘야하고 // BAO-c1 ABO-c1 AAO-c1 BBO-c1 OBO OAO 00A-c0 
					{
						countNone++; // 처음 공백위치 
						...
						else if (countNone == 2) // 두번 만남 A0A0    AA0AA0 00AA 00A0A0 끊기조건 처음 POINT위치로 다시 세기 
						{
							...
							else if (y - 1 >= 0 && (Game[y - 1 + i][y - 1] == 'W'))
							{
								list[startPoint + i][startPoint].max = list[y - 1 + i][y - 1].combo; // 이전BW
								y = nonePoint;
								continue;
				...
				if (list[y + i][y].combo == 3) // 3일때 
				{
					...
					for (int j = startPoint; j <= endPoint; j++) // 최대 1~5일때 10234
					{
						if (Game[j + i][j] == '+') //공백있을때
						{
							...
							if (list[startPoint + i][startPoint].state == 'B')
							{
								Whelp[j + i][j] = 'o'; // 공백있는데 검은색일때 -> 백돌 맵에 그리기 
							}
							if (list[startPoint + i][startPoint].state == 'W')
							{
								Bhelp[j + i][j] = 'o';
							}
						}
					...
					if (list[startPoint + i][startPoint].state == 'B') //whelp 처음이 검은색일때
					{
						if (startPoint - 1 >= 0 && (Game[startPoint - 1 + i][startPoint - 1] == '+')) // 공백일때
						{
							Whelp[startPoint - 1 + i][startPoint - 1] = 'o'; // 막아!
						}
						if (endPoint + 1 <= 18 && (Game[endPoint + 1 + i][endPoint + 1] == '+')) // 공백일때
						{
							Whelp[endPoint + 1 + i][endPoint + 1] = 'o'; // 막아!
						}
					}
					else if (list[startPoint + i][startPoint].state == 'W') // bhelp 처음이 백돌일때 
					{
						if (startPoint - 1 >= 0 && (Game[startPoint - 1 + i][startPoint - 1] == '+')) // 공백일때
						{
							Bhelp[startPoint - 1 + i][startPoint - 1] = 'o'; // 막아! -> B도와주는 맵 
						}
						if (endPoint + 1 <= 18 && (Game[endPoint + 1 + i][endPoint + 1] == '+')) // 공백일때
						{
							Bhelp[endPoint + 1 + i][endPoint + 1] = 'o'; // 막아!
						}
					}
				}
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (y < 19 && i + y < 19)
			{
				if (list[y + i][y].max > max) max = list[y + i][y].max;
			}
		}
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (y < 19 && i + y < 19) {
				...
				if (max > 1 && (list[y + i][y].max == max))
				{
					printf("좌하단 %02d번째 연속 돌 좌표 ", k);  // 공백 출력 안해도 될듯 

					if (list[y + i][y].state == 'B')
					{
						printf("B(Y,X) : ");
						for (int j = y; j <= y + list[y + i][y].max; j++)
						{
							if (j <= 18 && (list[j + i][j].state == 'B')) // 공백포함 max까지하는데 돌멩이 잇을때만 출력 k=2, max=4-> 
							{
								printf("(%d,%d)", list[j + i][j].Y, list[j + i][j].X);
							}
						}
					}

					if (list[y + i][y].state == 'W')
					{
						printf("W(Y,X) : ");
						for (int j = y; j <= y + list[y + i][y].max; j++)
						{
							if (j <= 18 && (list[j + i][j].state == 'W'))
							{
								printf("(%d,%d)", list[j + i][j].Y, list[j + i][j].X);
							}
						}
	...
	for (int i = 18, k = 0; i >= 0; i--, k++)
	{
		...
		for (int y = 1; y < 2 * k; y++)
		{
			if (18 - y >= 0 && i + y < 19) // Game[0+y][i+y]  i 18~0, k0 ++, y=0~2*k y가 증가하는 줄 
			{
				if (y - 1 == 0) // 맨 처음일때 
				{
					if (Game[18 - y + 1][i + y - 1] == 'B') // y는 1이고 y-1이 0인 상황이 맨 처음 
					{
						countB = 1;
						sumB = countB;
						startPoint = y - 1;
						endPoint = y - 1;
						list[18 - y + 1][i + y - 1].state = 'B'; // 이전꺼 
						list[18 - y + 1][i + y - 1].Y = 18 - y + 1;
						list[18 - y + 1][i + y - 1].X = y + i - 1;
						list[18 - y + 1][i + y - 1].combo = 1;
					}
					else if (Game[18 - y + 1][i + y - 1] == 'W')
					{
						countW = 1;
						sumW = countW;
						startPoint = y - 1;
						endPoint = y - 1;
						list[18 - y + 1][i + y - 1].state = 'W';
						list[18 - y + 1][i + y - 1].Y = 18 - y + 1;
						list[18 - y + 1][i + y - 1].X = i + y - 1;
						list[18 - y + 1][i + y - 1].combo = 1;
					}
					else if (Game[18 - y + 1][i + y - 1] == '+')
					{
						nonePoint = y - 1;
						list[18 - y + 1][i + y - 1].state = '+';
						list[18 - y + 1][i + y - 1].Y = 18 - y + 1;
						list[18 - y + 1][i + y - 1].X = i + y - 1;
					}
				}
				...
				if (Game[18 - y][i + y] == Game[18 - y + 1][i + y - 1]) // 이전과 같을 때 BB ++ WW  기본형 Game[i][y].Game[0+y][i+y]
				{
					...
					else if (Game[18 - y][i + y] == 'W')
					{
						...
						endPoint = y;
						if (countW > sumW) sumW = countW;
						list[18 - y][i + y].state = 'W';
						list[18 - y][i + y].Y = 18 - y;
						list[18 - y][i + y].X = y + i;
						list[18 - y][i + y].combo = sumW;
					}
					else if (Game[18 - y][y + i] == '+') // 공백 2이상연속 nonepoint부터 돌멩이 나올때까지 k 조정 
					{
						if (y - 2 >= 0 && (Game[18 - y + 2][y + i - 2] == 'B'))
						...
						else if (y - 2 >= 0 && (Game[18 - y + 2][y + i - 2] == 'W'))
						{
				...
				{
					if (Game[18 - y][i + y] == 'B') // 전에 꺼 비교해서 설정해주기 
					{
						if (y - 1 >= 0 && (Game[18 - y + 1][y + i - 1] == 'W'))
						{
						...
						else  if (y - 1 >= 0 && (Game[18 - y + 1][y + i - 1] == '+')) // AOB BOB 
						{
							if (countNone == 1) // 공백 하나일때 연속중 
							{
								countB++;
					...
					else if (Game[18 - y][i + y] == 'W')
					{
						if (y - 1 >= 0 && (Game[18 - y + 1][y + i - 1] == 'B'))
						{
							...
							countNone = 0;
							startPoint = y;
							list[18 - y][y + i].state = 'W';
							list[18 - y][y + i].Y = 18 - y;
							list[18 - y][y + i].X = y + i;
							list[18 - y][y + i].combo = sumW;
						}
						else  if (y - 1 >= 0 && (Game[18 - y + 1][y + i - 1] == '+'))
						{
							if (countNone == 1) // 공백 하나일때 연속중  A0A 
							{
								countW++;
					...
					else if (Game[18 - y][y + i] == '+') // coutnone 2면 끊어줘야하고 // BAO-c1 ABO-c1 AAO-c1 BBO-c1 OBO OAO 00A-c0 
					{
						countNone++; // 처음 공백위치 
						...
						else if (countNone == 2) // 두번 만남 A0A0    AA0AA0 00AA 00A0A0 끊기조건 처음 POINT위치로 다시 세기 
						{
							...
							else if (y - 1 >= 0 && (Game[18 - y + 1][y + i - 1] == 'W'))
							{
								list[18 - startPoint][startPoint + i].max = list[18 - y + 1][y + i - 1].combo; // 이전BW
								y = nonePoint;
								continue;
				...
				if (list[18 - y][y + i].combo == 3) // 3일때 
				{
					...
					for (int j = startPoint; j <= endPoint; j++) // 최대 1~5일때 10234
					{
						if (Game[18 - j][j + i] == '+') //공백있을때
						{

							if (list[18 - startPoint][startPoint + i].state == 'B')
							{
								Whelp[18 - j][j + i] = 'o'; // 공백있는데 검은색일때 -> 백돌 맵에 그리기 
							}
							if (list[18 - startPoint][startPoint + i].state == 'W') // st == y / y-1][y-1+i 
							{
								Bhelp[18 - j][j + i] = 'o';
							}
						}
					...
					//
					if (list[18 - startPoint][i + startPoint].state == 'B') //whelp 처음이 검은색일때
					{
						if (startPoint - 1 >= 0 && (Game[18 - startPoint + 1][startPoint + i - 1] == '+')) // 공백일때
						{
							Whelp[18 - startPoint + 1][startPoint + i - 1] = 'o'; // 막아!
						}
						if (endPoint + 1 <= 18 && (Game[18 - endPoint - 1][endPoint + i + 1] == '+')) // 공백일때
						{
							Whelp[18 - endPoint - 1][endPoint + i + 1] = 'o'; // 막아!
						}
					}
					else if (list[18 - startPoint][i + startPoint].state == 'W') // bhelp 처음이 백돌일때 
					{
						if (startPoint - 1 >= 0 && (Game[18 - startPoint + 1][startPoint + i - 1] == '+')) // 공백일때
						{
							Bhelp[18 - startPoint + 1][startPoint + i - 1] = 'o'; // 막아! -> B도와주는 맵 
						}
						if (endPoint + 1 <= 18 && (Game[18 - endPoint - 1][endPoint + i + 1] == '+')) // 공백일때
						{
							Bhelp[18 - endPoint - 1][endPoint + i + 1] = 'o'; // 막아!
						}
					}
				}
				else if (list[18 - y][y + i].combo == 4) // 콤보 3또는 4일때 
				{
					...
					for (int j = startPoint; j <= endPoint; j++) // 최대 1~5일때 10234 // k를 j로 k는 y임 
					{
						if (Game[18 - j][j + i] == '+') //공백있을때
						{
							...
							if (list[18 - startPoint][startPoint + i].state == 'W')
							{
								Bhelp[18 - j][j + i] = 'o';
								//	if (Bhelp[startPoint - 1][startPoint - 1 + i] == 'o') Bhelp[startPoint - 1][startPoint - 1 + i] = '+';
								//	if (Bhelp[endPoint + 1][endPoint + 1 + i] == 'o') Bhelp[endPoint + 1][endPoint + 1 + i] = '+';
					...
					if (blanckOn == 0) // 공백 없을때
					{
						if (list[18 - startPoint][startPoint + i].state == 'B') //whelp 처음이 검은색일때
						{
							if (startPoint - 1 >= 0 && (Game[18 - startPoint + 1][startPoint + i - 1] == '+')) // 공백일때
							{
								Whelp[18 - startPoint + 1][startPoint + i - 1] = 'o'; // 막아!
						...
						else if (list[18 - startPoint][18 - startPoint + i].state == 'W') // bhelp 처음이 백돌일때 
						{
							if (startPoint - 1 >= 0 && (Game[18 - startPoint + 1][startPoint + i - 1] == '+')) // 공백일때
							{
								Bhelp[18 - startPoint + 1][startPoint + i - 1] = 'o'; // 막아! -> B도와주는 맵 
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (18 - y >= 0 && i + y < 19)
			{
				if (list[18 - y][i + y].max > max) max = list[18 - y][y + i].max;
			}
		}
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (18 - y >= 0 && i + y < 19) {
				...
				if (max > 1 && (list[18 - y][y + i].max == max))
				{
					printf("우하단 %02d번째 연속 돌 좌표 ", k);  // 공백 출력 안해도 될듯 

					if (list[18 - y][y + i].state == 'B')
					{
						printf("B(Y,X) : ");
						for (int j = y; j <= y + list[18 - y][y + i].max; j++)
						{
							if (j <= 18 && (list[18 - j][j + i].state == 'B')) // 공백포함 max까지하는데 돌멩이 잇을때만 출력 k=2, max=4-> 
							{
								printf("(%d,%d)", list[18 - j][j + i].Y, list[18 - j][j + i].X);
							}
						}
					}

					if (list[18 - y][y + i].state == 'W')
					{
						printf("W(Y,X) : ");
						for (int j = y; j <= y + list[18 - y][y + i].max; j++)
						{
							if (j <= 18 && (list[18 - j][j + i].state == 'W'))
							{
								printf("(%d,%d)", list[18 - j][j + i].Y, list[18 - j][j + i].X);
							}
						}
					...
					if (list[18 - y][y + i].state != 0) printf("\n");
				}
			}
		}
	}
	...
	for (int i = 18, k = 0; i > 0; i--, k++)
	{
		...
		for (int y = 1; y < 2 * k; y++)
		{
			if (k - y >= 0 && y < 18) // Game[0+y][i+y]  i 18~0, k0 ++, y=0~2*k y가 증가하는 줄 
			{
				if (y - 1 == 0) // 맨 처음일때 
				{
					if (Game[k - y + 1][y - 1] == 'B') // y는 1이고 y-1이 0인 상황이 맨 처음 
					{
						countB = 1;
						sumB = countB;
						startPoint = y - 1;
						endPoint = y - 1;
						list[k - y + 1][y - 1].state = 'B';
				...
				if (Game[k - y][0 + y] == Game[k - y + 1][y - 1]) // 이전과 같을 때 BB ++ WW  기본형 Game[i][y].Game[0+y][i+y]
				{
					...
					else if (Game[k - i][y + 0] == '+') // 공백 2이상연속 nonepoint부터 돌멩이 나올때까지 k 조정 
					{
						if (y - 2 >= 0 && (Game[k - y + 2][y - 2] == 'B'))
						{
							list[k - startPoint][startPoint].max = list[k - y + 2][y - 2].combo; // 연속 공백 나오면 끊어야하는데 전에 있던거 max에 백업해야지 
						}
						else if (y - 2 >= 0 && (Game[k - y + 2][y - 2] == 'W'))
						{
				...
				{
					if (Game[k - y][y] == 'B') // 전에 꺼 비교해서 설정해주기 
					{
						if (y - 1 >= 0 && (Game[k - y + 1][y - 1] == 'W'))
						{
						...
						else  if (y - 1 >= 0 && (Game[k - y + 1][y - 1] == '+')) // AOB BOB 
						{
							if (countNone == 1) // 공백 하나일때 연속중 
							{
								countB++;
					...
					else if (Game[k - y][y] == '+') // coutnone 2면 끊어줘야하고 // BAO-c1 ABO-c1 AAO-c1 BBO-c1 OBO OAO 00A-c0 
					{
						countNone++; // 처음 공백위치 
						...
						else if (countNone == 2) // 두번 만남 A0A0    AA0AA0 00AA 00A0A0 끊기조건 처음 POINT위치로 다시 세기 
						{
							...
							else if (y - 1 >= 0 && (Game[k - y + 1][y - 1] == 'W'))
							{
								list[k - startPoint][startPoint].max = list[k - y + 1][y - 1].combo; // 이전BW
								y = nonePoint;
								continue;
				...
				if (list[k - y][y].combo == 3) // 3일때 
				{
					...
					for (int j = startPoint; j <= endPoint; j++) // 최대 1~5일때 10234
					{
						if (Game[k - j][j] == '+') //공백있을때
						{
							...
							if (list[k - startPoint][startPoint].state == 'B')
							{
								Whelp[k - j][j] = 'o'; // 공백있는데 검은색일때 -> 백돌 맵에 그리기 
							}
							if (list[k - startPoint][startPoint].state == 'W')
							{
								Bhelp[k - j][j] = 'o';
							}
						}
					...
					if (list[k - startPoint][startPoint].state == 'B') //whelp 처음이 검은색일때
					{
						if (startPoint - 1 >= 0 && (Game[k - startPoint + 1][startPoint - 1] == '+')) // 공백일때
						{
							Whelp[k - startPoint + 1][startPoint - 1] = 'o'; // 막아!
						}
						if (endPoint + 1 <= 18 && (Game[k - endPoint - 1][endPoint + 1] == '+')) // 공백일때
						{
							Whelp[k - endPoint - 1 + i][endPoint + 1] = 'o'; // 막아!
						}
					}
					else if (list[k - startPoint][startPoint].state == 'W') // bhelp 처음이 백돌일때 
					{
						if (startPoint - 1 >= 0 && (Game[k - startPoint + 1][startPoint - 1] == '+')) // 공백일때
						{
							Bhelp[k - startPoint + 1][startPoint - 1] = 'o'; // 막아! -> B도와주는 맵 
						}
						if (endPoint + 1 <= 18 && (Game[k - endPoint - 1][endPoint + 1] == '+')) // 공백일때
						{
							Bhelp[k - endPoint - 1][endPoint + 1] = 'o'; // 막아!
						}
					}
				}
				else if (list[k - y][y].combo == 4) // 콤보 3또는 4일때 
				{
					...
					if (blanckOn == 0) // 공백 없을때
					{
						if (list[k - startPoint][startPoint].state == 'B') //whelp 처음이 검은색일때
						{
							...
							if (endPoint + 1 <= 18 && (Game[k - endPoint - 1][endPoint + 1] == '+')) // 공백일때
							{
								Whelp[k - endPoint - 1][endPoint + 1] = 'o'; // 막아!
							}
						}
						else if (list[k - startPoint][startPoint].state == 'W') // bhelp 처음이 백돌일때 
						{
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (k - y >= 0 && y < 18)
			{
				if (list[k - y][y].max > max) max = list[k - y][y].max;
			}
		}
		...
		for (int y = 0; y < 2 * k; y++)
		{
			if (k - y >= 0 && y < 18) {
				...
				if (max > 1 && (list[k - y][y].max == max))
				{
					printf("좌상단 %02d번째 연속 돌 좌표 ", k);  // 공백 출력 안해도 될듯 

					if (list[k - y][y].state == 'B')
					{
						printf("B(Y,X) : ");
						for (int j = y; j <= y + list[k - y][y].max; j++)
						{
							if (j <= 18 && (list[k - j][j].state == 'B')) // 공백포함 max까지하는데 돌멩이 잇을때만 출력 k=2, max=4-> 
							{
								printf("(%d,%d)", list[k - j][j].Y, list[k - j][j].X);
							}
						}
					}

					if (list[k - y][y].state == 'W')
					{
						printf("W(Y,X) : ");
						for (int j = y; j <= y + list[k - y][y].max; j++)
						{
							if (j <= 18 && (list[k - j][j].state == 'W'))
							{
								printf("(%d,%d)", list[k - j][j].Y, list[k - j][j].X);
							}
						}
